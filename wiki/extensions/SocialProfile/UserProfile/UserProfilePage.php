...
class UserProfilePage extends Article {
	...
	function getPersonalInfo( $user_id, $user_name ) {
		...
		$level_link = Title::makeTitle( NS_HELP, 'm:User Levels' ); //GEORGE default--> NS_HELP, wfMsgForContent( 'user-profile-userlevels-link' )
	...
	/**
	 * 
	 * Rewrite of wgLang->pipeList that wraps the pipe in a span to allow CSS formatting
	 * @param unknown_type $list
	 */
	function pipeList( array $list ) {
		return implode(
			'<span>' . wfMessage( 'pipe-separator' )->inLanguage( $this )->escaped() . '<span>',
			$list
		);
	}
	...
	function getProfileTop( $user_id, $user_name ) {
		...
		if ( $this->isOwner() ) {
			$output .= $wgLang->pipeList( array(
				'<a href="/wiki/Special:Preferences" rel="nofollow">' . wfMsg( 'preferences' ) . '</a>',
				'<a href="' . $update_profile->escapeFullURL() . '">' . wfMsg( 'user-edit-profile' ) . '</a>',
				'<a href="http://meta.brickimedia.org/wiki/Special:UploadAvatar">' . wfMsg( 'user-upload-avatar' ) . '</a>', //GEORGE
				#Default: '<a href="' . $upload_avatar->escapeFullURL() . '">' . wfMsg( 'user-upload-avatar' ) . '</a>',
			...
			if ( $relationship == false ) {
				$output .= $wgLang->pipeList( array(
					'<a href="/wiki/User_talk:' . $user . '" rel="nofollow">Talk Page</a>',
					...
					//NXT trying to get rid of foes, sorry about the hardcoding...
					''
				) );
			} else {
				if ( $relationship == 1 ) {
		...
		$output .= wfMsgExt( 'pipe-separator', 'escapenoentities' );
		$output .= '<a href="/wiki/Special:Editcount/' . $user . '" rel="nofollow">Editcount</a> ';
		...
		if ( $wgTitle->getNamespace() == NS_USER_PROFILE && $this->profile_data['user_id'] && $this->profile_data['user_page_type'] == 0 ) {
			$output .= '<span>|</span> <a href="' . $user_page->escapeFullURL() . '" rel="nofollow">' .
				wfMsg( 'user-page-link' ) . '</a> ';
		}

		// Links to User:user_name from User_profile:
		if ( $wgTitle->getNamespace() == NS_USER && $this->profile_data['user_id'] && $this->profile_data['user_page_type'] == 0 ) {
			$output .= '<span>|</span> <a href="' . $user_social_profile->escapeFullURL() . '" rel="nofollow">' .
		...
		if ( $wgTitle->getNamespace() == NS_USER && ( !$this->profile_data['user_id'] || $this->profile_data['user_page_type'] == 1 ) ) {
			$output .= '<span>|</span> <a href="' . $user_wiki->escapeFullURL() . '" rel="nofollow">' .
