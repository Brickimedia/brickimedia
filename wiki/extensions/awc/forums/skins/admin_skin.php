...
class awc_admin_skin  extends awcforum_forumAdmin{
    ...
    function css_editing_boxs($css_info, $css_id){
       ...
       # awc_pdie($css_info);
    	...
    	#awc_pdie($css_info);
        if($css_info['css_att'][0] == "#" || $css_info['css_att'][0] == ".") {
            $link = substr($css_info['css_att'],1,strlen($css_info['css_att'])); 
        } else {
            $link = $css_info['css_att'];
        }
         
        $link = str_replace(array('}', '[', ']', '{',), '', $link);
        
        if($css_info['custom'] == 0){
            $html = "<a href='http://wiki.anotherwebcom.com/$link (Forum CSS)' target='_blank'>{$css_info['css_att']}</a> ".get_awcsforum_word('admin_edit_ver') . " {$css_info['ver']}<br />" ;
        } else {
            $html = $css_info['css_att'] ."  ".get_awcsforum_word('admin_edit_ver') . " {$css_info['ver']} <b>" .get_awcsforum_word('word_custom'). '</b><br />';
        }
