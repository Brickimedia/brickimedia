...
* SVN  Revision: $Revision: 538 $
* Last Modified: $Date: 2011-01-18 02:16:40 -0500 (Tue, 18 Jan 2011) $
* 
*/
       ...
       # $e['html'] = null;
		...
		while ($r = $dbr->fetchObject( $res )) {
		 ++$i;
                 ...
                 #if($i == 1){
                 	#$html .= "|PLACE_MARKER_FOR_POSTING_BOX|";
                 #}
        ...
        if(!$single_post AND (strlen($sql2) == 0)){  
           ...
           # $wgOut->addHTML($html);
            ...
            if(((strlen($sql2) == 0) AND $awcUser->canPost AND $this->forum_perm_can_post AND $t_status == "0" AND !$single_post) || (UserPerm >= 2 && $i AND !$single_post)){
                ...
                $postBoxForm = $posting_box->box();
                unset($posting_box);
                
               # $html = str_replace("|PLACE_MARKER_FOR_POSTING_BOX|", $postBoxForm, $html);
               # $wgOut->addHTML($html);
            }
            
            # $html = str_replace("|PLACE_MARKER_FOR_POSTING_BOX|", $postBoxForm, $html);
            $wgOut->addHTML($html . $postBoxForm);
	return ;	
	}
