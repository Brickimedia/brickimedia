	...
	'fn': {
		'trackClick': function( id ) {
			if ( tracked && typeof $.trackActionWithInfo == 'function' ) {
				$.trackActionWithInfo( prefix( id ), mw.config.get( 'wgTitle' ) + '|' + mw.config.get( 'wgCurRevisionId' ) );
			}
		},
		/**
				...
				'success': function( data ) {
					...
					context.$ui.find( '.articleFeedback-rating' ).each( function() {
						...
						if (
							...
							var average = rating.total / rating.count;
							average = average.toFixed(1);
							$(this)
								.find( '.articleFeedback-rating-average' )
									.text( mw.language.convertNumber( average, false ) )
									.end()
					...
					.click( function() {
						...
						for ( var key in context.options.pitches ) {
							...
							if ( display !== 'hide' && context.options.pitches[key].condition() ) {
								var weight = 'weight' in context.options.pitches[key] ? context.options.pitches[key].weight : 1;
