...
class SpecialArticleFeedback extends UnlistedSpecialPage {
	...
	public function execute( $par ) {
		...
		$wgOut->addStyle( 'ext.articleFeedback.dashboard' );
		$this->setHeaders();
		if ( $wgArticleFeedbackDashboard ) {
			...
			$this->renderDailyHighsAndLows(
				$highs,
				$this->msg( 'articleFeedback-table-caption-dailyhighs', $wgLang->date( time() ) )->text()
			);

			//render daily lows table
			$this->renderDailyHighsAndLows(
				$lows,
				$this->msg( 'articleFeedback-table-caption-dailylows', $wgLang->date( time() ))->text()
			);
	...
	protected function renderDailyHighsAndLows( $pages, $caption ) {
			...
			foreach ( $pages as $page ) {
				...
				foreach ( $categories as $catid => $catmsg ) {
					if ( isset( $page['ratings'][$catid] ) ) {
						...
						$number = $this->msg( 'articlefeedback-table-noratings' )->text();
						$class = 'none';
					}
					$row[] = array(
						'text' => $number,
		...
		$this->renderTable(
			$caption,
			array_merge(
				array( $this->msg( 'articleFeedback-table-heading-page' )->text() ),
				$categories,
				array( $this->msg( 'articleFeedback-table-heading-average' )->text() )
			),
			$rows,
	...
	protected function renderWeeklyMostChanged() {
		...
		$this->renderTable(
			$this->msg( 'articleFeedback-table-caption-weeklymostchanged' )->text(),
			array_merge(
				array( $this->msg( 'articleFeedback-table-heading-page' )->text() ),
				$categories
			),
	...
	protected function renderProblems() {
		...
		$this->renderTable(
			$this->msg( 'articleFeedback-table-caption-recentlows' )->text(),
			array_merge(
				array( $this->msg( 'articleFeedback-table-heading-page' )->text() ),
				$categories,
				array( $this->msg( 'articleFeedback-table-heading-average' )->text() )
	...
	protected static function getCategories() {
		...
		if ( !isset( self::$categories ) ) {
			foreach ( $wgArticleFeedbackRatingTypes as $id => $rating ) {
				self::$categories[$id] = wfMessage( "articlefeedback-field-$rating-label" )->text();
			}
		}
