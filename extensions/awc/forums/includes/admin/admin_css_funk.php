...
class awc_admin_css_cls extends awcforum_forumAdmin{
    ...
    global $wgOut; 
        ...
        switch( $todo ) {
          ...
          case 'css_edit_save_all':
                $done = self::css_edit_save_all();
                if($done == true) $wgOut->redirect( awc_url . 'admin/css/css_get_list' );
                break;
     ...
     * @since 2.6.0
     */
    public function editForm($info, $sectionDropdown){
    	
    	$out = $sectionDropdown . '<br />';
    	$out .= awcsforum_funcs::awc_htmlentities($info['css_att']) . '<br />';
    	$out .= '<textarea name="code" rows="40" cols="90%">' ;
    	$out .= awcsforum_funcs::awc_htmlentities($info['css_code']) ;
    	$out .= '</textarea><br />' ;
    ...
    public function sectionDropdown(){
        ...
        $dbr = &wfGetDB( DB_SLAVE );
        $awc_f_theme_css = $dbr->tableName( 'awc_f_theme_css' );  
        $sql = "SELECT css_section
                FROM $awc_f_theme_css 
                WHERE css_id={$this->css_id}
                GROUP BY css_section";
        
        $res = $dbr->query($sql);
        
        $out = '<select name="sectionDropdown[]">';
        while ($r = $dbr->fetchObject( $res )) {
        	$out .= '<option value="' . $r->css_section . '">' . $r->css_section . '</option>';
        }
        $out .= '</select>';
                ...
                if(in_array($r->thmn_id, $awcs_forum_config->cf_css_active_ids)) {
                   $active = 'CHECKED';
                    ...
                    $out .= ' <input name="id" type="hidden" value="'.$r->thmn_id.'">' ;
                        ...
                        $out .=  ' <input type="submit" name="editall" value="'.get_awcsforum_word('editallcssatts').'"> ';
    ...
    function save_css_to_file($redirect = true){
    global $wgOut;    
        ...
        if($hd = fopen($filename, "w")){
	        fwrite($hd, $out);
	        fclose($hd);
	        if($redirect == false) return 0;
	        $wgOut->redirect( awc_url . 'admin/css/saved_css_list' );
        } else {
        	return awcs_forum_error(get_awcsforum_word('nofolderwriteperrmission') . '<br />' . awc_dir . 'skins/css/');
        }
    ...
    private function css_edit_save_all(){
    global $wgRequest, $awcs_forum_config;
    
    	#awc_pdie($wgRequest);
    	$css_thmn_id = $wgRequest->getVal( 'id' ) ;
    	$code = $wgRequest->getVal( 'code' ) ;
    	$codeArray = explode('}', $code);
    	
        $dbr = &wfGetDB( DB_SLAVE );
        $awc_f_theme_css = $dbr->tableName( 'awc_f_theme_css' );  
        $sql = "SELECT css_id, css_att, css_code, css_ver
                FROM $awc_f_theme_css WHERE css_thmn_id =" . $css_thmn_id;
                
        $res = $dbr->query($sql);
        while ($r = $dbr->fetchObject( $res )) {
        	
        	foreach($codeArray as $id => $codeChunk){
        		
        		$splt = explode('{', $codeChunk);
        		
        		if(isset($splt[1]) && $splt[0] != ''){
        		
	        		$att = $splt[0];
	        		$code = $splt[1];
	        		
	        		if(trim($att) == trim($r->css_att)){
	        			unset($codeArray[$id]);// remove so we can find new entries
	        			
	        			if(trim($code) != trim($r->css_code)){
	        				self::css_edit_save(false, $r->css_id, $code, $r->css_ver);
	        		 	}
        ...
        /* @TODO rework
         * The above loop will delete att - checks as the match to dbase entries
         * What ever left "has" to be new, so add then to the dbase
         * 
         * Add a "form page" here to give an option for which 'section' the att should be added to
         */
        array_pop($codeArray);
        if(!empty($codeArray)){
        	
        	#$out = '<form action="'. awc_url .'admin/css/css_save_all_new" method="post">';
        	#$out .= '<input name="id" type="hidden" value="'.$css_thmn_id.'">' ;
        	#$sectionDropdown = $this->sectionDropdown();
        	#$out .= $sectionDropdown;
        	...
        	foreach($codeArray as $id => $codeChunk){
        		...
        		$insert_new_value['css_section'] = 'custom_added';
				$insert_new_value['css_att'] = $att;
				$insert_new_value['css_code'] = $code;
				$insert_new_value['css_custom'] = 1;
				$insert_new_value['css_thm_id'] = 1;
				$insert_new_value['css_thmn_id'] = $css_thmn_id;
				
				#$out .= $this->editForm($insert_new_value, $sectionDropdown);
        		$this->insert_new_value($insert_new_value);
        		
        		
        		/* find a way to enter 'css_section'
					$dbw->insert( 'awc_f_theme_css', 
                                        array('css_att'       => $att,
                                                'css_thmn_id'   => $css_thmn_id,
                                                'css_code'      => $code,
                                                'css_thm_id'    => 1,
                                                'css_custom'    => 1,
                                                'css_ver'       => '1.0.0',
        ...
        if($awcs_forum_config->cf_css_fileorembed == 1) {
        	self::save_css_to_file(false);
        }
    ...
    function css_edit_save($redirect = true, $id = false, $code = false, $ver = false){
     global $wgOut, $ADskin, $wgRequest; 
        ...
        $css_id = ($id) ? $id : $wgRequest->getVal( 'id' ) ;
        $css_code = ($code) ? $code :$wgRequest->getVal( 'css_code' ) ;
        $css_ver = ($ver) ? $ver :$wgRequest->getVal( 'ver' ) ;
            ...
            while ($r = $dbr->fetchObject( $res )) {
               ...
               // $code = $wgRequest->getVal('tplt_code');
                ...
                $dbw->update( 'awc_f_theme_css', 
                            array('css_code' => trim($css_code), 'css_ver' => $css_save,), 
                            array('css_id' => $css_id), '');
    ...
    private function edit_all_css(){
    global $ADskin, $wgOut;
   # die(">". $this->css_id);
        $dbr = &wfGetDB( DB_SLAVE );
        $awc_f_theme_css = $dbr->tableName( 'awc_f_theme_css' );  
        $sql = "SELECT css_id, css_att, css_custom, css_code, css_ver , css_section
                FROM $awc_f_theme_css WHERE css_thmn_id =" . $this->css_id;
                
        $out = '<form action="'. awc_url .'admin/css/css_edit_save_all" method="post">';
        $out .= '<input name="id" type="hidden" value="'.$this->css_id.'">' ;
        $out .= '<textarea name="code" rows="40" cols="90%">' ;
    ...
    function css_edit_section(){
     global $wgOut, $ADskin, $wgRequest;
        ...
        $editall = ($wgRequest->getVal( 'editall' ));
        if($editall != null) return self::edit_all_css();
    ...
    /*
     * No check is ATT already exists, direct insert
     */
    function insert_new_value($infoArray){
    global $awcs_forum_config ;
    
    $css_section = $infoArray['css_section'];
    $css_att = $infoArray['css_att'];
    $css_code = $infoArray['css_code'];
    $css_custom = $infoArray['css_custom'];
    $css_thmn_id = $infoArray['css_thmn_id'];
    $css_thm_id = $infoArray['css_thm_id'];
      
    $dbw = &wfGetDB( DB_MASTER );
        
               // used to print info for update page...
    $out = "<b><u>CSS</u> New Attribute:</b><br /> - Attribute=".awcsforum_funcs::awc_htmlentities($css_att)."<br /> - Code=".awcsforum_funcs::awc_htmlentities($css_code)."<br /><br />";
