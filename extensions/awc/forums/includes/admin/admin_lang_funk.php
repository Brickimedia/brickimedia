               ...
               # ksort($lang);     
                 foreach ($lang as $k => $v) {
                     #die(print_r($lang_raw));
                        ...
                        $value = str_replace($repl, '', @$lang_raw[$k]);
                        $value = awcsforum_funcs::awc_htmlspecialchars($value);
                        $html .= "<b>'". $k . "'</b> " . $v . '<br /><input name="'.$k.'" type="text" value="'. $value .'" size="100%"> ';
                        $html .= '<br /><br /></td></tr>'; 
               }
            }
    ...
    function lang_do_export_lang(){
     global $awcs_forum_config ; 
            ...
            if($hd) {
            	...
            	global $wgOut;
            		$out=null;
             		$out = get_awcsforum_word('nofolderwriteperrmission');
             		$wgOut->addHTML('<br />' . $out . '<br /><b>' . awc_dir . 'xported_files/</b><br /><br />');
             	return false;
            }
