  ...
  //  var $displaysmiles;
   ...
   // var $poll_info;
    ...
    global $wgRequest;
        ...
        $this->tID = (int)$wgRequest->getVal( 'tID' );
        $this->fID = (int)$wgRequest->getVal( 'fID' ); 
        $this->pID = (int)$wgRequest->getVal( 'pID' );
        $this->t_wiki_pageid = (int)$wgRequest->getVal( 't_wiki_pageid' );
	...
	global $wgOut, $awcUser, $wgRequest, $wgParser, $wgTitle, $awc_tables;
     ...
     $rDB = &wfGetDB( DB_SLAVE );
        ...
        if($this->isThread  == "yes") {
            ...
            /* Added 2.5.12 */
             $wDB->update( 'awc_f_forums',
                        array(  'f_lasttitle'    => $this->title), 
                        array('f_threadid' => $this->tID),
                        '' );
