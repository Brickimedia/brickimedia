				...
				if ( $overview->wff_forum > 0 && !wfReadOnly() ) {
					...
					if ( $overview->wff_announcement == false || $wgUser->isAllowed( 'wikiforum-moderator' ) ) {
						...
						if ( $doublepost === false ) {
							...
							$logEntry = new ManualLogEntry( 'forum', 'create' ); // Action bar in log foo
							$logEntry->setPerformer( $wgUser ); // User object, the user who did this action
							$logEntry->setTarget( Title::newFromText( "Special:WikiForum/" . $title) ); // The page that this log entry affects
							$logEntry->setComment( substr( $text, 0, 40 ) . '...' ); // User provided comment, optional
		...
		if ( $wgWikiForumAllowAnonymous || $wgUser->isLoggedIn() ) {
			if ( $threadId > 0 ) {
				if ( strlen( $text ) > 1 ) {
					...
					$thread = $dbr->selectRow(
						'wikiforum_threads',
						array('wft_thread', 'wft_thread_name'),
						array(
							'wft_deleted' => 0,
							'wft_closed' => 0,
							'wft_thread' => intval( $threadId )
					...
					if ( $thread->wft_thread > 0 && !wfReadOnly() ) {
						...
						if ( $doublepost === false ) {
							...
							$logEntry = new ManualLogEntry( 'forum', 'comment' ); // Action bar in log foo
							$logEntry->setPerformer( $wgUser ); // User object, the user who did this action
							$name = $thread -> wft_thread_name;
							$logEntry->setTarget( Title::newFromText( "Special:WikiForum/" . $name) ); // The page that this log entry affects
							$logEntry->setComment( substr( $text, 0, 20 ) . '...' ); // User provided comment, optional
							$logEntry->setParameters( array(
									//'4::paramname' => 'customparam',
									//'hiddenparam' => 'ugly stuff',
							) );
